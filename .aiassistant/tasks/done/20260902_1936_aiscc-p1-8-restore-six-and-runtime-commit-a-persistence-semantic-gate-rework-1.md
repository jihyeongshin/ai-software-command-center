# 작업지시서: P1-8 restore six and Runtime Commit A persistence — semantic gate rework

## meta

- task_id: `20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1`
- created_at: `2026-09-02T19:36:00+09:00`
...
- fresh_chat_reason: `1849 ran in the genuinely new IDE chat created for this same destructive restore + Commit A authority class and performed no runtime/index/commit mutation; this Task only corrects the task-local semantic proof contract`
- predecessor_task: `.aiassistant/tasks/done/20260902_1849_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-fresh-ide-session-retry-2.md`
- predecessor_judgment_cycle: `.aiassistant/records/aiscc/cycles/20260902_1934_aiscc-p1-8-runtime-commit-a-six-semantic-equivalence-contract-rework-1.cycle.md`
...
- target_bundle: `.aiassistant/reports/target/20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1/`

- browser_session_continuation: `HUMAN_PROVIDED — current Browser Command Center continues; 1849 blocked result received substantive judgment and no Browser Handoff is required`
- ide_session_status: `1849 EXECUTED_PASS — current IDE chat is the genuinely fresh session created for this authority class; reuse is allowed because 1849 performed no runtime/index/commit mutation`
...
1849 Task는 genuinely new IDE Executor chat에서 실행되었고 transport/session/repository/identity gate는 모두 PASS했다.

```text
1849 Executor result:
...
1934 Command Center judgment:

```text
ACCEPTED_AS_ACCURATE_BLOCKED_RESULT
/ TASK_NOT_COMPLETED
/ SEMANTIC_GATE_REWORK_REQUIRED
```
...
governance after 1849 Task lifecycle: 17 Git-visible paths
accepted 36 aggregate: a82d94c1d607bc379c12d0768ff54d0cd481467eb731e0884f63176bd1207f3c
Commit A: NOT_CREATED
```

1849 exact six identity `6/6`, Python AST equivalence `6/6` PASS. strict token/comment gate만 `5/6`.
...
AST: PASS
strict diff hunks: exactly 2
HEAD-only OP '(' at range 227:228
HEAD-only OP ')' at range 229:230
other normalized token/comment differences: none reported
```

1934 Cycle은 위 exact fingerprint에 대해서만 task-local `REDUNDANT_PARENTHESIS_ONLY_EQUIVALENCE`를 허용한다.
Human exact-36 acceptance와 `RESTORE_SIX_TO_EXACT_HEAD` authority는 변경되지 않는다.

1849 IDE chat은 이 destructive authority class를 위해 만들어진 fresh session이고 mutation이 없었으므로 재사용 가능하다.
...
```text
C:\Users\oracl\Downloads\20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1.md
C:\Users\oracl\Downloads\20260902_1934_aiscc-p1-8-runtime-commit-a-six-semantic-equivalence-contract-rework-1.cycle.md
```

Repository destinations:

```text
.aiassistant/tasks/active/20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1.md
...
1934 Cycle expected SHA-256:

```text
e10b67526f142fb2ae2de6ef16318c7a6c03f5bd87da1a9176e2805e29b96366
```
...
1934 Cycle source: MUST exist
1934 Cycle destination: MUST NOT exist
1934 Cycle SHA-256: MUST equal e10b67526f142fb2ae2de6ef16318c7a6c03f5bd87da1a9176e2805e29b96366
```

하나라도 실패하면 두 파일 모두 Move하지 말고 alternate path search 없이 `TRANSPORT_PRECONDITION_FAILED`로 STOP한다.

### 4.2 transport execution

전체 precheck PASS 후 두 파일만 exact destination으로 `Move`, not Copy 한다.
post-move destination identity와 Downloads source absence를 확인한다.
기존 1621/1759 predecessor/Handoff에는 transport/delete action을 하지 않는다.

## 5. fresh IDE session precondition


1849 IDE session은 `SESSION_AUTHORITY EXECUTED_PASS`를 얻은 genuinely fresh chat이며, 이 Task는 같은
destructive restore + Runtime Commit A authority class의 proof-contract rework다.

```text
NEW CHAT REQUIRED: No
CURRENT 1849 FRESH IDE CHAT REUSE: Allowed
```

Pass:
- current conversation lineage가 1849 fresh-session과 일치
- 1849 이후 Task 밖 runtime/index/commit mutation evidence 없음
- 이 신규 Task를 새로운 execution authority로 읽음
...
transport/session precondition PASS 뒤 exact path로 읽는다.
...
Tool/config only when exact Ruff/mypy invocation requires:

- `pyproject.toml`

unrelated rule/record/source/log bulk-read 금지.
...
1849 lifecycle 종료 후 admitted governance count는 `17 Git-visible paths`다.

```text
A. transport 전: expected exact 17
B. 1934 Cycle + active Task transport 직후: expected exact 18
   - 1934 Cycle tracked
   - active Task ignored
C. active→done 직후: expected exact 19
   - current done Task tracked
```

이번 transport 후 다음이 존재해야 한다.

```text
.aiassistant/records/aiscc/cycles/20260902_1934_aiscc-p1-8-runtime-commit-a-six-semantic-equivalence-contract-rework-1.cycle.md
.aiassistant/tasks/active/20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1.md  # ignored
```

unexpected governance path가 있으면 allowlist/count를 확장하지 말고 mutation 전에 STOP한다.
...
```text
.aiassistant/reports/target/20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1/
```
...
- initial_state: `P1-8_RUNTIME_HUMAN_ACCEPTED / RESTORE_SIX_AUTHORIZED / COMMIT_A_PENDING / SEMANTIC_GATE_REWORK_AUTHORIZED`
- after transport/session/preflight: `COMMIT_A_PRECONDITIONS_VERIFIED`
- after corrected semantic gate: `SIX_SEMANTIC_EQUIVALENCE_PASS`
- after restore: `SIX_RESTORED / ACCEPTED_36_ONLY`
- after staging: `COMMIT_A_READY`
- success end: `COMMIT_A_CREATED / COMMAND_CENTER_REVIEW_REQUIRED`
...
- `PUBLIC_PROVENANCE / TRANSPORT`: new Task + 1934 Cycle exact Move
- `SESSION_AUTHORITY`: 1849 fresh-session lineage reuse
- `STATIC_SOURCE / LOCAL_GIT_PREFLIGHT`: exact branch/HEAD/index/runtime/governance
- `STATIC_SOURCE / RESTORE_IDENTITY`: six `6/6`
- `STATIC_SOURCE / SEMANTIC_EQUIVALENCE`: five strict + exact models.py narrow exception contract
- `STATIC_SOURCE / POST_RESTORE`: six clean, index empty, runtime 42→36
- `STATIC_SOURCE / ACCEPTED_RUNTIME_IDENTITY`: accepted 36 + aggregate exact
- `STATIC_CHECK`: diff-check + local Ruff/mypy
- `GIT_INDEX`: exact 36 only
- `GIT_OBJECT`: one exact Runtime Commit A
- `SOURCE_EVIDENCE_EXPORT`: manifest + exact Commit A tree copies

### reuse_allowed

- 1100 accepted runtime tests only after six restored to exact HEAD and accepted 36 unchanged
- 1222 Human exact-36 acceptance only while byte identities unchanged
- 1355 `RESTORE_SIX_TO_EXACT_HEAD` only while exact six current/HEAD identities unchanged and corrected gate PASS
- 1849 failure fingerprint defines the allowed exception but must be freshly recomputed

### human_owned

- future substantive Human/Command Center review of exact Commit A
...
- accepted runtime modification
- general parenthesis ignore/normalization
...
1849 report != fresh recomputation
...
4. six identity `6/6`
5. six AST `6/6`
6. five strict token/comment PASS
7. models.py strict PASS or exact narrow exception PASS
8. exact six restore; runtime 42→36; index empty
9. accepted 36 + aggregate exact
10. diff-check/Ruff/mypy PASS
11. stage exact 36 only
12. exactly one Commit A with exact parent/message/path/blob/tree
13. six/governance/config absent from Commit A
14. index empty
15. Commit A tree export/manifest integrity PASS
16. active→done complete
17. final result `COMMIT_A_CREATED / COMMAND_CENTER_REVIEW_REQUIRED`
...
- Task/1934 Cycle transport conflict/hash mismatch
...
- any AST mismatch
- strict mismatch on any of five strict paths
- models.py mismatch differs in any way from Section 10.3 exact fingerprint
- accepted 36 identity/aggregate mismatch
- static tool requires install/network

Pre-commit STOP:

- post-restore set not exact accepted 36
- diff/Ruff/mypy fail
- staging not exact 36
- Git author unavailable without config mutation

Post-commit discrepancy requires Command Center review; no repair second commit.
