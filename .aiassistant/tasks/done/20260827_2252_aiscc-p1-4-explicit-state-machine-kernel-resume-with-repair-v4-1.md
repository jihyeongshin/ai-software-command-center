# 작업지시서: P1-4 Explicit State Machine Kernel Resume with Repair Input v4

## meta

- task_id: `20260827_2252_aiscc-p1-4-explicit-state-machine-kernel-resume-with-repair-v4-1`
...
- primary_semantic_owner: `repair-input v4 + expanded P1-4 preflight provenance → P1-4 authoritative kernel`
...
The previous Task finally verified/applied the nested repair ZIP but could not commit because:

1. its own prior blocked done copy was outside the exact commit universe;
2. exact v3 `DECISION_REGISTER.md` bytes failed `git diff --check` due one extra blank line at EOF.

This Task repairs only those Stage 0 contract defects, then continues the unchanged P1-4 contract.

---

# Human preparation

Human extracts one outer repo-root placement packet at:
...
.aiassistant/bootstrap-input/20260827_2252_aiscc-p1-4-terminal-state-repair-input-v4-1.zip
```

Do not manually extract the nested repair ZIP.
Do not manually edit canonical files.
...
both files exist
AND
SHA-256(active bytes) == SHA-256(done bytes)
AND
both exact task_id values == 20260827_2252_aiscc-p1-4-explicit-state-machine-kernel-resume-with-placement-packet-1
```

This cleanup is ignored transport cleanup, not Git provenance mutation.
...
Do not delete the done copy.

---

# Stage 0B — exact repair-input v4
...
```text
a18a4c874d05b485f6188b2900ae7667e108b93c1f7197791e8d0465f4023c63
```

Exact inventory: seven files, no more/no less.
...
- missing ZIP;
- SHA mismatch;
- sixth/eighth or otherwise wrong entry count;
- absolute/path-traversal/alternate-root entry;
- unexpected symlink-like entry.

Extract to ignored temp only.

Verify every extracted hash before canonical copy.

Copy exactly the seven canonical paths.

Verify every post-copy canonical hash.

For `DECISION_REGISTER.md`, additionally verify:

```text
file ends with exactly one LF
file does not end with two LF bytes
```
...
If exact v4 hashes pass but `git diff --check` still fails:
...
Do not edit beyond v4 bytes.

---

# Stage 0C — semantic gate
...
four blocker Cycles
→ all describe P1-4 pre-implementation Stage 0 blockers
```

P1-3 terminal Cycle remains required and unchanged.

---

# Stage 0D — expanded exact terminal/preflight provenance commit
...
Ignored active Tasks, repair ZIPs, temp extraction, target report are never staged.
...
Recompute P1-3 aggregate immediately before commit.
...
Record all blocked P1-4 preflights and resume P1-4 from the verified
...
- committed paths inside exact 68-path universe;
- committed P1-3 aggregate exact;
...
P1-4 implementation stays uncommitted for review.
...
```text
.aiassistant/reports/target/20260827_2252_aiscc-p1-4-explicit-state-machine-kernel-resume-with-repair-v4-1/
```
