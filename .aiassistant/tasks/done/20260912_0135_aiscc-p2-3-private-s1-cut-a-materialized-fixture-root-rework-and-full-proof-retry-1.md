# 작업지시서: P2-3 private S1 Cut A materialized fixture root rework + full proof retry

## meta

- task_id: `20260912_0135_aiscc-p2-3-private-s1-cut-a-materialized-fixture-root-rework-and-full-proof-retry-1`
...
Correct only the bounded materialized-result fixture root mismatch proven by the
`0125` integration result.

Do not modify production workspace/materialized-result validation.
...
```text
.aiassistant/tasks/active/20260912_0135_aiscc-p2-3-private-s1-cut-a-materialized-fixture-root-rework-and-full-proof-retry-1.md
```
...
SHA-256:
38270df63036b9a443bf5f00bcc627514b77bc448d6531de55eccf18af1b5424

.aiassistant/reports/aiscc/20260912_0135_aiscc-p2-3-cut-a-materialized-fixture-root-mismatch-judgment-1.md
SHA-256:
2b5185af191929ff517518bc599332fe7f848189021394fdc6734ad18d71efdd
```
...
Before this delivery exact Git-visible set is 49:
...
Require exact current 19-path draft:
...
The other 18 Cut A paths are frozen:
...
Any scope need beyond this test:
...
and change only the bounded-owner-return lease field:

```python
runtime_root=tmp_path,
```

at the `StockroomWorkspaceLease(...)` construction near the bounded materialized result
to:
...
Do NOT change the later intentional negative/tampered fixture:
...
Do not create/write anything under `private_runtime_root`.
...
# 6. fixture-boundary proof

Before calling the bounded `capture.adapter.materialize(...)`, prove from test-local
objects:

```text
private_runtime_root exists
private_runtime_root is empty

bounded_owner_return.resolved_source_root == private_runtime_root
bounded_owner_return.workspace_lease.runtime_root == private_runtime_root
bounded_owner_return.workspace_lease.destination == private_runtime_root

test-only provenance JSON:
outside private_runtime_root

fake Docker executable:
outside private_runtime_root
```

Do not weaken the production checks to make this pass.

# 7. static proof

Do not use `py_compile`.

In-memory compile the same exact 14 Cut A Python paths used by `0125`.
...
Run the corrected external V1/V2 static verifier from the `0125` contract:
...
For the corrected bounded production test, explicitly record:

```text
application private runtime root:
empty before construction

bounded materialized destination:
private_runtime_root

bounded lease runtime root:
private_runtime_root
...
no real Stockroom Docker/provider/tool execution

private runtime root:
empty at final assertion
```

Also preserve all later intentional negative/tampered materialized-output cases.

If another independent failure occurs:
...
Require all prior Cut A contract rows PASS:
...
The test path is already visible, so its corrected bytes add no path.
...
No same-turn source repair after a mandatory static/test failure.
...
```text
.aiassistant/reports/target/20260912_0135_aiscc-p2-3-private-s1-cut-a-materialized-fixture-root-rework-and-full-proof-retry-1/
```
...
`EXPORT_MANIFEST.md` covers all 38 non-self entries.
