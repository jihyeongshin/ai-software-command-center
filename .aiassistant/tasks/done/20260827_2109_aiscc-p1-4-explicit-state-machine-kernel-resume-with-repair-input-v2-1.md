# 작업지시서: P1-4 Explicit State Machine Kernel Resume with Exact Terminal Repair Input v2

## meta

- task_id: `20260827_2109_aiscc-p1-4-explicit-state-machine-kernel-resume-with-repair-input-v2-1`
...
- primary_semantic_owner: `exact terminal repair-input v2 → P1-3 terminal/preflight provenance commit → authoritative P1-4 state-machine kernel`
...
The predecessor Task stopped before canonical mutation because the required ignored repair ZIP was
not present.

This Task is the only active P1-4 execution contract.

It does not reopen P1-3 and does not redesign the P1-4 contract.

Execution sequence:

```text
repair-input v2 exact verification
→ exact five-file canonical repair
→ one P1-3 terminal + two blocked P1-4 preflight provenance commit
→ same-turn P1-4 implementation
```

---

# Human preparation — mandatory before Executor starts

Human must place this exact ZIP **without extracting it** at:

```text
.aiassistant/bootstrap-input/20260827_2109_aiscc-p1-4-terminal-state-repair-input-v2-1.zip
```
...
```text
0267d4b780203f87359d2450f904349d6a7f4ac2486297e767bf7d6a86c67581
```

Also place this Task at:

```text
.aiassistant/tasks/active/20260827_2109_aiscc-p1-4-explicit-state-machine-kernel-resume-with-repair-input-v2-1.md
```

Do not separately copy/extract the Cycle files.
Do not manually repair canonical state files.
Do not create a Git commit.

---

# Stage 0A — repository and accepted-candidate preflight
...
Verify index empty and run `git diff --check`.

Any HEAD or accepted-candidate mismatch:
...
Do not repair P1-3 product source.

---

# Stage 0B — exact repair-input v2 verification
...
ZIP inventory must be exactly these five files and no others:
...
- absolute paths;
- path traversal / `..`;
- alternate canonical roots;
- unexpected symlink-like entry.

Extract only to an ignored temporary directory.

Verify each extracted file hash before copying.

Then copy exactly those five files to their exact repository canonical paths.

Verify canonical post-copy hash for each file.
...
Do not reconstruct or merge stale repository wording.

---

# Stage 0C — semantic terminal gate

After v2 repair, verify:
...
prior conflict Cycle
→ partial canonical application blocker

missing-input Cycle
→ exact repair ZIP absent blocker
```

Also verify the P1-3 terminal Cycle remains canonical:
...
# Stage 0D — one terminal/preflight provenance commit
...
Every actual staged path must be in this universe.

The ignored v2 ZIP, temp extraction directory, active Task, and target report are never staged.
...
Recompute P1-3 accepted aggregate immediately before commit.
...
Record both blocked P1-4 preflights, including the missing repair-input
attempt, then resume P1-4 from the verified authoritative repository.
```
...
- every committed path belongs to the exact 64-path universe;
- committed P1-3 candidate aggregate exact;
- exact multiline message;
- tracked worktree clean;
- index empty;
- no remote operation.

Delete only the ignored temporary extraction directory after validation.
...
```text
.aiassistant/reports/target/20260827_2109_aiscc-p1-4-explicit-state-machine-kernel-resume-with-repair-input-v2-1/
```
