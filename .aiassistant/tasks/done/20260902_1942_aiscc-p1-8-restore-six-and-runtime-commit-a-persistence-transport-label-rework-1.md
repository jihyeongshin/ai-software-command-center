# 작업지시서: P1-8 restore six and Runtime Commit A persistence — transport label consistency rework

## meta

- task_id: `20260902_1942_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-transport-label-rework-1`
- created_at: `2026-09-02T19:42:00+09:00`
...
- fresh_chat_reason: `1849 established the genuinely fresh IDE session; 1849 and 1936 performed no runtime/index/commit mutation; this Task stays in the same destructive authority class and only fixes current-Cycle transport/report labels`
- predecessor_task: `.aiassistant/tasks/done/20260902_1936_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-semantic-gate-rework-1.md`
- predecessor_judgment_cycle: `.aiassistant/records/aiscc/cycles/20260902_1940_aiscc-p1-8-runtime-commit-a-transport-authority-label-conflict-rework-1.cycle.md`
...
- target_bundle: `.aiassistant/reports/target/20260902_1942_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-transport-label-rework-1/`

- browser_session_continuation: `HUMAN_PROVIDED — current Browser Command Center continues; 1936 blocked result received substantive judgment and no Browser Handoff is required`
- ide_session_status: `1849 EXECUTED_PASS + 1936 NO-MUTATION BLOCK — current IDE chat remains the genuinely fresh session for this authority class and may be reused`
...
1936 Task는 같은 genuinely fresh IDE Executor session에서 실행되었고 exact Task + 1934 Cycle transport까지 PASS했다.

그 뒤 active Task 내부에서 current provenance Cycle label conflict가 발견되어 mutation 전에 중단되었다.

```text
1936 result:
POLICY_CONFLICT_INVESTIGATION_REQUIRED / TRANSPORT_AUTHORITY_LABEL_CONFLICT
...
1940 Command Center judgment:

```text
ACCEPTED_AS_ACCURATE_BLOCKED_RESULT
/ TASK_NOT_COMPLETED
/ TRANSPORT_AUTHORITY_LABEL_REWORK_REQUIRED
```
...
governance after 1936 Task lifecycle: 19 Git-visible paths
Runtime Commit A: NOT_CREATED
```

1936 blocker는 semantic gate 실패가 아니다. fresh Section 9/10 gate는 conflict 이후 실행되지 않았다.

1934에서 승인된 corrected semantic gate는 그대로 유지한다:

- five strict paths: AST + strict normalized token/comment equality
- exact `src/aiscc/evidence/models.py`: strict equality 또는 exact `REDUNDANT_PARENTHESIS_ONLY_EQUIVALENCE` fingerprint
- 모든 proof는 이번 Task에서 current worktree와 exact HEAD bytes로 fresh recomputation

Human exact-36 acceptance와 `RESTORE_SIX_TO_EXACT_HEAD` authority는 변경되지 않는다.

현재 IDE chat은 1849에서 genuinely fresh session으로 입증됐고 1849/1936 모두 runtime/index/commit mutation이 없었다.
따라서 새 chat은 필요하지 않다.

## 2. 이번 턴 목표


1. Downloads의 **신규 1942 Task와 1940 judgment Cycle 두 파일만** exact transport contract에 따라 canonical 위치로 Move한다.
2. 현재 fresh IDE session lineage와 local repository branch/HEAD/index/runtime/governance identity를 mutation 전에 재검증한다.
3. exact six current bytes와 expected HEAD bytes가 Human-authorized identity와 일치하는지 검증한다.
4. exact six에 대해 Section 10 corrected semantic-equivalence gate를 current bytes와 exact HEAD bytes로 fresh recomputation한다.
...
10. exact contract로 Runtime Commit A를 정확히 하나만 생성한다.
11. Commit A object parent/message/path/blob/tree를 검증하고 Commit A tree에서 exact 36 source copies를 export한다.
12. index가 empty인지 확인하고 `COMMIT_A_CREATED / COMMAND_CENTER_REVIEW_REQUIRED`로 종료한다.
...
이번 transport 대상은 정확히 두 파일뿐이다.

```text
C:\Users\oracl\Downloads\20260902_1942_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-transport-label-rework-1.md
C:\Users\oracl\Downloads\20260902_1940_aiscc-p1-8-runtime-commit-a-transport-authority-label-conflict-rework-1.cycle.md
```
...
1940 Cycle expected SHA-256:

```text
328a6ca2da22f6b4d2b6aab3e1fb9449d56d3d5dec4f8f7bf70bec31e4b8d027
```

### 4.1 transport precheck

```text
new Task source: MUST exist
new Task destination: MUST NOT exist
1940 Cycle source: MUST exist
1940 Cycle destination: MUST NOT exist
1940 Cycle source SHA-256: MUST equal 328a6ca2da22f6b4d2b6aab3e1fb9449d56d3d5dec4f8f7bf70bec31e4b8d027
```
...
1. 신규 1942 Task를 exact active destination으로 `Move`, not Copy.
2. 1940 Cycle을 exact cycles destination으로 `Move`, not Copy.
3. 두 destination identity와 두 Downloads source absence를 확인한다.
4. 기존 1934/1759/1621 predecessor Cycle/Handoff에는 transport/delete action을 수행하지 않는다.

**이 Task의 모든 current transport/report authority label은 1940 Cycle이다.**

## 5. fresh IDE session precondition


1849 IDE session은 `SESSION_AUTHORITY EXECUTED_PASS`를 얻은 genuinely fresh chat이다.
1936은 같은 session에서 Task-authority conflict로 runtime/index/commit mutation 전에 STOP했다.
...
- current conversation lineage가 1849→1936 fresh-session chain과 일치
- 1936 이후 Task 밖 runtime/index/commit mutation evidence 없음
- 이 신규 1942 Task를 새로운 execution authority로 명시적으로 읽음
...
Executor는 새 chat을 스스로 만들지 않는다.

## 6. 읽을 문서 — minimum authoritative context set


transport/session precondition PASS 뒤 destructive action 전에 exact path로 읽는다.
...
> 1934 judgment Cycle + accepted predecessor Cycles/Handoff
...
Current local canonical이 1936 blocked result 이후 합법적으로 advance한 흔적이 있으면 `git reset`, checkout, restore로 predecessor HEAD에 맞추지 않는다.
...
1936 lifecycle 종료 후 admitted governance count는 `19 Git-visible paths`다.

세 시점을 정확히 분리한다.

```text
A. transport 전: expected exact 19 Git-visible governance paths
B. 1940 Cycle + current active Task transport 직후: expected exact 20
   - 1940 Cycle tracked
   - current active Task ignored
C. Task 완료 시 active→done lifecycle 직후: expected exact 21
   - current Task done path tracked 추가
```

A에는 1936 done Task와 1934 Cycle이 반드시 포함되어야 한다.

이번 transport 후 추가 canonical 존재:

```text
.aiassistant/records/aiscc/cycles/20260902_1940_aiscc-p1-8-runtime-commit-a-transport-authority-label-conflict-rework-1.cycle.md
.aiassistant/tasks/active/20260902_1942_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-transport-label-rework-1.md  # ignored active lifecycle path
```

unexpected Git-visible governance path가 있으면 allowlist/count를 자동 확장하지 않는다.
exact identity를 report하고 mutation 전에 STOP한다.
...
```text
.aiassistant/reports/target/20260902_1942_aiscc-p1-8-restore-six-and-runtime-commit-a-persistence-transport-label-rework-1/
```
...
- initial_state: `P1-8_RUNTIME_HUMAN_ACCEPTED / RESTORE_SIX_AUTHORIZED / COMMIT_A_PENDING / TRANSPORT_LABEL_REWORK_AUTHORIZED`
- after current Task/1940 Cycle transport + session lineage + exact preflight: `COMMIT_A_PRECONDITIONS_VERIFIED`
...
- `PUBLIC_PROVENANCE / TRANSPORT`: exact new 1942 Task + 1940 Cycle Move
- `SESSION_AUTHORITY`: 1849→1936 fresh-session lineage reuse
...
- 1934 corrected semantic-gate definition; actual gate must be recomputed in this Task
- 1100 accepted tests only under the unchanged accepted-36 + exact-HEAD-restored-six applicability condition
...
1934/1849 evidence != fresh Section 10 recomputation
...
1. exact 1942 Task + 1940 Cycle transport PASS
2. session lineage PASS
3. repository preflight PASS
4. six identity `6/6`
5. Section 10 corrected semantic gate PASS
6. exact six restore; runtime 42→36; index empty
7. accepted 36 + aggregate exact
8. diff-check/Ruff/mypy PASS
9. stage exact 36 only
10. exactly one Runtime Commit A with exact parent/message/path/blob/tree
11. six/governance/config absent from Commit A
12. index empty
13. Commit A tree export/manifest integrity PASS
14. active→done complete
15. final result `COMMIT_A_CREATED / COMMAND_CENTER_REVIEW_REQUIRED`
...
- Task/1940 Cycle transport conflict/hash mismatch
- any current Task clause refers to a different current judgment Cycle for transport/report authority
...
Post-commit discrepancy requires Command Center review; do not create a repair second commit.
...
- Downloads transport precheck and exact action for **current 1942 Task + 1940 Cycle**
...
- six AST/token/comment equivalence result including exact models.py narrow-exception fingerprint if used
...
- all other predecessor accepted/terminal Task/Cycle artifacts already tracked by canonical repository

Superseded/non-canonical Browser Handoff that remains outside this Task:

```text
20260902_1759_aiscc-browser-command-center-session-handoff-p1-8-runtime-commit-a-human-fresh-chat-retry-resume-1.md
```

Executor does not transport/delete it.
