# P3-3 Public Live Limiter Retention / Resource-Bound Policy Proposal V1
...
It does not change the already accepted request-rate caps:
...
Use the authoritative database-clock fixed-minute bucket already defined by accepted shared-limit policy.
...
The accepted V1 flood order currently consumes the campaign dimension and the source dimension for every ingress attempt.

Amend only the already-globally-denied case:
...
This is an explicit V1 amendment.
...
This amendment therefore narrows resource consumption without weakening admission.
...
If no Public Live traffic occurs, no new limiter rows are created; deferred cleanup is therefore acceptable until the next limiter operation.
...
Do not silently skip overdue maintenance and continue Public Live admission.
...
Human acceptance of this document alone does NOT resolve the release blocker.

After implementation, PostgreSQL proof, regression, Git persistence, and Browser Command Center acceptance:
...
Only then may later Public admission/release gates treat limiter persistent storage as bounded.
