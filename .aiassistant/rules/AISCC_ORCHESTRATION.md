...
Authorized implementation/testing does not authorize an actual golden run. A later
Task must separately authorize the first operational Genesis enrollment, TaskContract,
READY, external IDE start, completion lease before edit, trusted submission, Evidence,
Judgment and Cycle. This amendment itself performs none of those operational actions.
Tests may exercise existing owner APIs in disposable PostgreSQL; their synthetic
guard capabilities are implementation proofs, never actual golden authority.
The accepted steady-state path remains CYCLE_DERIVED after the first owner-admitted
Cycle; no restart or revoked context reopens Genesis. Browser review remains required
